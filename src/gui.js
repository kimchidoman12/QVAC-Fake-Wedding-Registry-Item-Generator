#!/usr/bin/env node
// QVAC Fake Wedding Registry Item Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29420;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/registryitem") {
      try {
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake Wedding Registry Item Generator GUI ready at http://localhost:${PORT}`);
  });
